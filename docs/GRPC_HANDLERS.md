...
| 5 | `ServerLoaderInfoHandler` | `ServerLoaderInfoRequest` | None | Returns real-time server load metrics: `sdkConCount` (active SDK connections), `cpu` (CPU usage %), `load` (1-min load average), `mem` (memory usage %). Uses `sysinfo` crate. |
| 6 | `ServerReloadHandler` | `ServerReloadRequest` | Authenticated | Validates the server configuration file (`conf/application.yml`). Does not hot-reload; validates file exists and contains required sections. Requires restart to apply. |
...
| 6 | `ConfigChangeClusterSyncHandler` | `ConfigChangeClusterSyncRequest` | Internal | Handles cluster-wide config change notifications from peer nodes. Logs the sync event with `dataId`, `group`, `tenant`, `lastModified`, `grayName`. Notifies local subscribers about the change. |
...
| 9 | `ConfigFuzzyWatchSyncHandler` | `ConfigFuzzyWatchSyncRequest` | Internal | Synchronizes fuzzy watch state across cluster nodes. Registers the watch pattern and tracks batch progress (`currentBatch`/`totalBatch`). |
...
| 5 | `SubscribeServiceRequestHandler` | `SubscribeServiceRequest` | None | Subscribes to or unsubscribes from service change notifications. When `subscribe=true`, registers the connection for push notifications. Returns current `ServiceInfo` immediately. Future instance changes are pushed via `NotifySubscriberRequest`. |
...
| 2 | `QueryMcpServerHandler` | `QueryMcpServerRequest` | Authenticated | Queries MCP server details by namespace and name. Returns the full server specification as JSON in the `detail` field, or an error if not found. |
...
| 5 | `QueryAgentCardHandler` | `QueryAgentCardRequest` | Authenticated | Queries an A2A agent card by namespace and name. Returns the full agent card as JSON in the `detail` field, or an error if not found. |
