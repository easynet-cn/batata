...
async fn test_console_server_serves_v2_health_readiness() {
    ...
    // Verify the response contains readiness-related structure
    if response["code"] == 0 {
        // When ready, data should be present (typically a string or null indicating ready state)
        assert!(
            response.get("data").is_some(),
            "Readiness response should contain a 'data' field when successful"
        );
    } else {
        // When not ready, message should describe the issue
        assert!(
            response.get("message").is_some(),
            "Readiness response should contain a 'message' field when not ready"
        );
    }
}
...
async fn test_console_server_serves_auth() {
    ...
    // Parse response body and verify it contains accessToken
