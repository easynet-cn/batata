...
/// Helper: extract config content from V2 GET response (handles both string and object data)
fn extract_config_content(response: &serde_json::Value) -> String {
    if response["data"].is_object() {
        ...
        response["data"].as_str().unwrap_or("").to_string()
    } else {
...
async fn test_config_special_characters_content() {
    ...
    // Retrieve and verify the exact content was preserved
    ...
    let content = extract_config_content(&response);
    assert_eq!(
        content, special_content,
        "Special characters content must be preserved exactly"
    );
}
...
async fn test_config_very_long_dataid() {
    ...
    let publish_content = "test.value=1";
    let _: serde_json::Value = client
        .post_form(
            "/nacos/v2/cs/config",
            &[
    ...
    assert_eq!(
        content, publish_content,
        "Config with long dataId must be retrievable with correct content"
    );
}
...
async fn test_config_very_long_content() {
    ...
    assert_eq!(
        content.len(),
        long_content.len(),
        "Long content length must be preserved (expected {}, got {})",
        long_content.len(),
        content.len()
    );
    assert_eq!(
        content, long_content,
        "Long content must be preserved exactly"
    );
}
...
async fn test_config_whitespace_content() {
    ...
    assert!(
        !content.is_empty(),
        "Whitespace content should be preserved (not empty)"
    );
    // Whitespace may be trimmed or preserved depending on server behavior;
    // at minimum verify the content contains whitespace characters
    assert!(
        content.contains(char::is_whitespace),
        "Retrieved content should contain whitespace characters"
    );
}
...
async fn test_service_special_characters() {
    ...
    // Verify the service has registered instances
    let hosts = response["data"]["hosts"].as_array();
    assert!(
        hosts.is_some() && !hosts.unwrap().is_empty(),
        "Service with special chars in name should have registered instances"
    );
    // Verify the service name is preserved in the response
    let svc_name = response["data"]["name"]
        .as_str()
        .or_else(|| response["data"]["serviceName"].as_str());
    if let Some(name) = svc_name {
        assert_eq!(
            name, service_name,
            "Service name with special characters must be preserved"
        );
    }
}
...
async fn test_instance_boundary_ip() {
    ...
    let hosts = response["data"]["hosts"]
        .as_array()
        .expect("Should have hosts array");
    assert!(
        hosts.len() >= 2,
        "Should have at least 2 registered instances, got {}",
        hosts.len()
    );
    ...
    assert!(
        ips.contains(&"0.0.0.0"),
        "Boundary IP 0.0.0.0 must be preserved, got: {:?}",
        ips
    );
    assert!(
        ips.contains(&"255.255.255.255"),
        "Boundary IP 255.255.255.255 must be preserved, got: {:?}",
        ips
    );
}
...
async fn test_instance_boundary_port() {
    ...
    // Collect all ports from the response
    let ports: Vec<i64> = hosts.iter().filter_map(|h| h["port"].as_i64()).collect();
    assert!(
        ports.contains(&1),
        "Boundary port 1 must be preserved, got: {:?}",
        ports
    );
    assert!(
        ports.contains(&65535),
        "Boundary port 65535 must be preserved, got: {:?}",
        ports
    );
}
...
async fn test_instance_boundary_weight() {
    ...
    // Collect all weights from the response
    let weights: Vec<f64> = hosts.iter().filter_map(|h| h["weight"].as_f64()).collect();
    assert!(
        weights.iter().any(|w| (*w - 0.1).abs() < 0.01),
        "Boundary weight 0.1 must be preserved, got: {:?}",
        weights
    );
    assert!(
        weights.iter().any(|w| (*w - 100.0).abs() < 0.01),
        "Boundary weight 100.0 must be preserved, got: {:?}",
        weights
    );
}
...
async fn test_instance_special_metadata() {
    ...
    assert!(
        !hosts.is_empty(),
        "Should have at least one registered instance"
    );

    // Verify metadata is preserved on the instance
    let instance = &hosts[0];
    let instance_metadata = &instance["metadata"];
    assert!(
        instance_metadata.is_object(),
        "Instance metadata should be an object, got: {:?}",
        instance_metadata
    );
    assert_eq!(
        instance_metadata["special.chars.key"], "special=value!@#",
        "Special chars in metadata value must be preserved"
    );
    assert_eq!(
        instance_metadata["unicode.key"], "测试中文🚀",
        "Unicode in metadata value must be preserved"
    );
}
...
async fn test_config_very_long_group() {
    ...
    assert_eq!(
        content, publish_content,
        "Config with long group name must be retrievable with correct content"
    );
}
