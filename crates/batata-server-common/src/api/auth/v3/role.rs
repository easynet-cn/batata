...
async fn search_page(
    ...
    match result {
        Ok(page) => {
            ...
            Result::<Page<RoleInfo>>::http_success(api_page)
        }
        Err(e) => HttpResponse::InternalServerError().json(Result::<Page<RoleInfo>> {
            code: 500,
            message: e.to_string(),
            data: Page::default(),
        }),
...
async fn search(
    ...
    match result {
        Ok(roles) => Result::<Vec<String>>::http_success(roles),
        Err(e) => HttpResponse::InternalServerError().json(Result::<Vec<String>> {
            code: 500,
            message: e.to_string(),
            data: Vec::new(),
        }),
