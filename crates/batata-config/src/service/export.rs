...
mod tests {
    use super::*;
    use crate::model::{ConfigInfo, ConfigInfoBase};

    fn create_test_config() -> ConfigAllInfo {
        ConfigAllInfo {
    ...
    fn test_create_nacos_export_zip() {
        ...
        assert!(archive.by_name("DEFAULT_GROUP/app.yaml").is_ok());
        // Check meta file exists
        assert!(archive.by_name("DEFAULT_GROUP/app.yaml.meta").is_ok());
    }
    ...
    fn test_config_to_export_item() {
        ...
        assert_eq!(item.metadata.data_id, "app.yaml");
        assert_eq!(item.metadata.group, "DEFAULT_GROUP");
        assert_eq!(item.metadata.namespace_id, "public");
        assert_eq!(item.content, "server:\n  port: 8080");
        assert_eq!(item.metadata.config_tags, "env:test");
    }
}
